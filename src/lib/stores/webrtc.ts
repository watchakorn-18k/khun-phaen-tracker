...
export const webrtcStatus = writable<
  "idle" | "connecting" | "connected" | "host" | "syncing" | "error"
>("idle");
...
export const incomingSyncData = writable<{
  type: string;
  document?: string;
  sprints?: any[];
} | null>(null);
...
export const incomingSyncRequest = writable<{
  peerId: string;
  timestamp: number;
} | null>(null);

// Store for new peer joined (host should sync to new peer)
export const newPeerJoined = writable<{
  peerId: string;
  timestamp: number;
} | null>(null);
    ...
    case "join-ack":
      if (!isHostPeer && data.targetPeer === get(myPeerId)) {
        ...
        connectedPeersList.set([
          data.peerId,
          ...data.peers.filter((p: string) => p !== get(myPeerId)),
        ]);

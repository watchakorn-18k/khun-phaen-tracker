<script lang="ts">
  ...
  const dispatch = createEventDispatcher<{
    ...
    statusChange: { id: number | string; status: Task["status"] };
    pageChange: number;
  ...
  function getStatusColor(
    ...
    switch (status) {
      ...
      case "in-test":
        return "bg-purple-100/50 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400 border-purple-300/30";
    }
  }
  ...
  let openMenuId: number | string | null | undefined = null;
</script>
            ...
            <div class="flex-1 min-w-0">
              ...
              >
                ...
                {#if getSprintName(task.sprint_id) && !task.is_archived}
                  <span
                    class="flex items-center gap-1 px-2 py-0.5 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 rounded text-xs"
                  >
                    <Flag size={12} />
                ...
                {#if task.is_archived && getSprintName(task.sprint_id)}
                  <span
                    class="flex items-center gap-1 px-2 py-0.5 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 rounded text-xs border border-gray-200 dark:border-gray-600"
                  >
                    <Archive size={12} />
                    {getSprintName(task.sprint_id)}
                  </span>

<script lang="ts">
  ...
  const dispatch = createEventDispatcher<{
    selectAssignee: { assigneeId: string | number | "all" | null };
  }>();
  ...
  function getWorkloadRows(
    ...
    const assigneeById = new Map<string | number, Assignee>(
      _assignees
        .filter(
          (assignee): assignee is Assignee & { id: string | number } =>
    ...
    for (const task of filtered) {
      ...
      const assigneeIds: (string | number | null)[] = [
        ...(task.assignee_ids ||
    ...
    for (const row of map.values()) {
      row.activeLoadScore =
        row.todo + row.inProgress * 2 + row.inTest * 1.5 + row.overdue * 3;
    }

    return [...map.values()].sort((a, b) => {
      if (b.activeLoadScore !== a.activeLoadScore)
        return b.activeLoadScore - a.activeLoadScore;
  ...
  function getTaskCount(workerId: string | number): number {
    const stat = workloadRows.find(
      (s) => String(s.assigneeId) === String(workerId),
    );
    return stat?.total || 0;
  }
...
<div class="space-y-4">
  ...
  <div
    class="flex flex-wrap items-center justify-between gap-4 bg-white dark:bg-gray-800 p-3 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm transition-colors"
  >
      ...
      <div class="flex p-1 bg-gray-100 dark:bg-gray-900 rounded-lg">
        {#each ["7d", "1m", "3m", "1y", "all", "custom"] as period}
          <button
            ...
            class="px-3 py-1.5 rounded-md text-xs font-medium transition-all {selectedPeriod ===
            period
  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
    <div
      class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 shadow-sm transition-colors"
    >
      <p class="text-xs text-gray-500 dark:text-gray-400">
        {$_("workload__total_people")}
      </p>
    ...
    >
      <p class="text-xs text-gray-500 dark:text-gray-400">
        {$_("workload__active_tasks")}
      </p>
      <p class="text-2xl font-semibold text-gray-900 dark:text-white">
        {workloadRows.reduce(
          (sum, row) => sum + row.todo + row.inProgress + row.inTest,
          0,
        )}
      </p>
    </div>
  ...
  <div
    class="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm transition-colors"
  >
    <table class="min-w-full text-sm">
      <thead
        class="bg-gray-50 dark:bg-gray-800/80 text-gray-600 dark:text-gray-300"
      >
        <tr>
          <th class="px-4 py-3 text-left font-medium"
            >{$_("workload__person")}</th
          >
          <th class="px-4 py-3 text-right font-medium"
            >{$_("workload__todo")}</th
          >
          <th class="px-4 py-3 text-right font-medium"
            >{$_("workload__in_progress")}</th
          >
          <th class="px-4 py-3 text-right font-medium"
            >{$_("workload__in_test")}</th
          >
          <th class="px-4 py-3 text-right font-medium"
            >{$_("workload__overdue")}</th
          >
          <th class="px-4 py-3 text-right font-medium"
            >{$_("workload__done")}</th
          >
          <th class="px-4 py-3 text-right font-medium"
            >{$_("workload__total")}</th
          >
          <th class="px-4 py-3 text-right font-medium"
            >{$_("workload__score")}</th
          >
        </tr>
      </thead>
      <tbody
        class="divide-y divide-gray-100 dark:divide-gray-700 bg-white dark:bg-gray-900/40"
      >
        {#if workloadRows.length === 0}
          <tr>
            <td
              colspan="8"
              class="px-4 py-12 text-center text-gray-500 dark:text-gray-400 bg-gray-50/30 dark:bg-gray-800/20"
            >
        ...
        {:else}
          {#each workloadRows as row}
            ...
            >
              ...
              >
              <td
                class="px-4 py-3 text-right text-purple-600 dark:text-purple-400"
                >{row.inTest}</td
              >
              ...
              >
              <td
                class="px-4 py-3 text-right text-emerald-600 dark:text-emerald-400"
                >{row.done}</td
              >
              <td class="px-4 py-3 text-right text-gray-700 dark:text-gray-300"
                >{row.total}</td
              >
              <td
                class="px-4 py-3 text-right font-bold {loadLevelClass(
                  row.activeLoadScore,
                )}"
              >

<script lang="ts">
  ...
  $: displayName = [$user?.profile?.first_name, $user?.profile?.last_name]
    .filter(Boolean).join(' ') || $user?.profile?.nickname || $user?.email?.split('@')[0] || '';
      ...
      <div class="grid gap-6 lg:grid-cols-[280px_minmax(0,1fr)]">
        ...
        <aside class="lg:sticky lg:top-0 lg:self-start">
          <div class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_18px_45px_rgba(15,23,42,0.08)] dark:border-gray-800 dark:bg-gray-800/50 dark:backdrop-blur-sm dark:shadow-none">
            <!-- Gradient Header -->
            <div class="border-b border-slate-200 bg-[radial-gradient(circle_at_top_left,_rgba(99,102,241,0.22),_transparent_38%),linear-gradient(135deg,_#f8fafc_0%,_#e2e8f0_45%,_#c7d2fe_100%)] p-5 text-slate-900 dark:border-gray-700 dark:bg-gradient-to-br dark:from-slate-900 dark:via-slate-800 dark:to-indigo-950 dark:text-white">
              <!-- Avatar -->
              <div class="mb-4 flex flex-col items-center gap-3">
                <div class="w-20 h-20 rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-black text-3xl shadow-xl shadow-indigo-500/30 ring-4 ring-white/20">
                  {initials}
                </div>
                <div class="text-center">
                  <p class="font-bold text-base tracking-tight truncate max-w-[200px]">{displayName || '—'}</p>
                  <p class="text-xs text-slate-500 dark:text-slate-400 truncate max-w-[200px] mt-0.5">{$user?.email || ''}</p>
                </div>
              </div>
            </div>
            ...
            <div class="p-4 space-y-3">
              <div class="flex items-center gap-3 rounded-2xl bg-gray-50 dark:bg-gray-800/60 px-4 py-3">
                <div class="rounded-xl bg-indigo-100 dark:bg-indigo-500/10 p-2 text-indigo-600 dark:text-indigo-400">
                  <ShieldCheck size={14} />
                </div>
                <div class="min-w-0">
                  <p class="text-[10px] font-bold uppercase tracking-widest text-gray-400 dark:text-gray-500">Role</p>
                  <p class="text-sm font-bold text-gray-900 dark:text-white capitalize">{$user?.role || '—'}</p>
                </div>
              </div>

              {#if nickname}
              <div class="flex items-center gap-3 rounded-2xl bg-gray-50 dark:bg-gray-800/60 px-4 py-3">
                <div class="rounded-xl bg-purple-100 dark:bg-purple-500/10 p-2 text-purple-600 dark:text-purple-400">
                  <AtSign size={14} />
                </div>
                <div class="min-w-0">
                  <p class="text-[10px] font-bold uppercase tracking-widest text-gray-400 dark:text-gray-500">{$_('users__form_nickname')}</p>
                  <p class="text-sm font-bold text-gray-900 dark:text-white truncate">{nickname}</p>
                </div>
              </div>
              {/if}
              ...
              {#if position}
              <div class="flex items-center gap-3 rounded-2xl bg-gray-50 dark:bg-gray-800/60 px-4 py-3">
                <div class="rounded-xl bg-emerald-100 dark:bg-emerald-500/10 p-2 text-emerald-600 dark:text-emerald-400">
                  <Fingerprint size={14} />
                </div>
                <div class="min-w-0">
                  <p class="text-[10px] font-bold uppercase tracking-widest text-gray-400 dark:text-gray-500">{$_('users__form_position')}</p>
                  <p class="text-sm font-bold text-gray-900 dark:text-white truncate">{position}</p>
                </div>
              </div>
              {/if}
            </div>
        ...
        <section class="min-w-0 space-y-6 animate-fade-in">
          ...
          <!-- Header Card -->
          <div class="overflow-hidden rounded-[30px] border border-slate-200 bg-white shadow-[0_30px_80px_rgba(15,23,42,0.12)] dark:border-slate-800/80 dark:bg-slate-950 dark:shadow-[0_30px_80px_rgba(15,23,42,0.45)]">
            <div class="bg-[radial-gradient(circle_at_top_left,_rgba(99,102,241,0.20),_transparent_35%),linear-gradient(135deg,_#f8fafc_0%,_#e2e8f0_48%,_#c7d2fe_100%)] px-6 py-7 dark:bg-[radial-gradient(circle_at_top_left,_rgba(99,102,241,0.28),_transparent_35%),linear-gradient(135deg,_#020617_0%,_#0f172a_48%,_#1e1b4b_100%)] sm:px-8">
              ...
              <p class="mt-2 max-w-xl text-sm leading-6 text-slate-600 dark:text-slate-300/85">
          ...
          <!-- Form Card -->
          <div class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_18px_45px_rgba(15,23,42,0.08)] dark:border-gray-800 dark:bg-gray-800/50">
            <form on:submit|preventDefault={handleSubmit} class="p-6 space-y-6">
              ...
              {#if saveSuccess}
                <div class="p-3 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-xl text-emerald-600 dark:text-emerald-400 text-sm font-medium">
              ...
              <!-- Email (read-only) -->
              <div>
                <label for="email" class="flex items-center gap-1.5 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1.5">
                  <Mail size={12} />
                ...
                <div class="relative">
                  ...
                  <div class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400"><Lock size={14} /></div>
                </div>
              </div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <div class="relative group">
                    <div class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-indigo-500 transition-colors"><User size={16} /></div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <div class="relative group">
                    <div class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-indigo-500 transition-colors"><AtSign size={16} /></div>
              ...
              <div>
                ...
                <div class="relative group">
                  ...
                  <input type="text" id="discordId" bind:value={discordId}
                    class="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all shadow-sm"
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <div class="relative group">
                    <div class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-indigo-500 transition-colors"><Lock size={16} /></div>

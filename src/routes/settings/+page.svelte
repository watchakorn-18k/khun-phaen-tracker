      ...
      <div class="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div class="min-w-0">
          ...
          <div class="flex flex-wrap items-center gap-3">
            ...
            <span class="rounded-full border border-emerald-500/20 bg-emerald-500/10 px-3 py-1 text-xs font-semibold text-emerald-700 dark:border-emerald-400/20 dark:bg-emerald-400/10 dark:text-emerald-200">
        ...
        <div class="grid w-full max-w-md grid-cols-2 gap-3">
          {#each summaryCards as card}
            <div class="rounded-2xl border border-white/70 bg-white/65 p-4 shadow-sm backdrop-blur dark:border-white/10 dark:bg-white/5 dark:shadow-none">
              <div class={`inline-flex rounded-xl bg-gradient-to-br p-2.5 ${card.tone}`}>
                <svelte:component this={card.icon} size={16} />
              </div>
              <p class="mt-3 text-xs font-semibold uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">
                {$_(card.label)}
              </p>

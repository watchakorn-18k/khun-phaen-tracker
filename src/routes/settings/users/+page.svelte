...
<div class="space-y-6 animate-fade-in">
        ...
        >
          <tr>
            <th
              ...
              >{$_("users__table_user")}</th
            >
            <th
              class="px-6 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider"
              >{$_("users__table_role")}</th
            >
            <th
              class="px-6 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider"
              >{$_("users__table_status")}</th
            >
            <th
              class="px-6 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider"
              >{$_("users__table_joined")}</th
            >
            <th
              class="px-6 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-right"
              >{$_("users__table_action")}</th
            >
          </tr>
        </thead>
        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
          {#if loading}
            {#each Array(3) as i}
              <tr class="animate-pulse">
                <td class="px-6 py-4"
                  ><div
                    class="h-10 w-48 bg-gray-200 dark:bg-gray-700 rounded"
                  ></div></td
                >
                <td class="px-6 py-4"
                  ><div
                    class="h-6 w-16 bg-gray-200 dark:bg-gray-700 rounded"
                  ></div></td
                >
                <td class="px-6 py-4"
                  ><div
                    class="h-6 w-20 bg-gray-200 dark:bg-gray-700 rounded"
                  ></div></td
                >
                <td class="px-6 py-4"
                  ><div
                ...
                <td class="px-6 py-4"
                  ><div
                    class="h-8 w-8 bg-gray-200 dark:bg-gray-700 rounded float-right"
                  ></div></td
                >
              </tr>
            {/each}
          {:else if error}
            <tr>
              <td
                colspan="5"
                class="px-6 py-12 text-center text-red-500 font-medium"
              >
                {error}
              </td>
            </tr>
            ...
            {#each filteredUsers as u}
              ...
              >
                <td class="px-6 py-4">
                  <div class="flex items-center gap-3">
                    <div class="relative">
                      ...
                      >
                        <div
                          class="w-2 h-2 rounded-full {u.is_active
                            ? 'bg-green-500'
                            : 'bg-amber-400'}"
                        ></div>
                      </div>
                    ...
                    <div>
                      ...
                      {#if u.discord_id}
                        ...
                        >
                          <div
                            class="w-2.5 h-2.5 rounded-full bg-indigo-500/20 flex items-center justify-center text-[6px] font-bold text-indigo-500"
                          >
                            D
                          </div>
                          {u.discord_id}
                ...
                <td class="px-6 py-4">
                  {#if u.role === "admin"}
                    <span
                      class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 text-[10px] font-bold uppercase tracking-wider border border-purple-200 dark:border-purple-800"
                    >
                      <Shield size={10} />
                      {$_("users__role_admin")}
                    </span>
                  {:else}
                    <span
                      class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 text-[10px] font-bold uppercase tracking-wider border border-gray-200 dark:border-gray-600"
                    >
                      <UserIcon size={10} />
                      {$_("users__role_user")}
                    </span>
                ...
                <td class="px-6 py-4">
                  {#if u.is_active}
                    <span
                      class="inline-flex items-center gap-1 text-green-500 text-xs font-medium"
                    >
                      <CheckCircle2 size={14} />
                      {$_("users__status_active")}
                    </span>
                  {:else}
                    <span
                      class="inline-flex items-center gap-1 text-amber-500 text-xs font-medium"
                    >
                      <Calendar size={14} />
                      {$_("users__status_pending")}
                    </span>
                  {/if}
                </td>
  ...
  {#if showDeleteConfirm}
    <div class="fixed inset-0 z-[1100] flex items-center justify-center p-4">
      ...
      ></button>
      <div
        class="relative w-full max-w-sm bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 p-6 animate-zoom-in"
      >
        <div class="flex flex-col items-center text-center">
          <div
            class="w-16 h-16 rounded-full bg-red-100 dark:bg-red-900/30 flex items-center justify-center text-red-500 mb-4 ring-8 ring-red-50 dark:ring-red-900/10"
          >
            <Trash2 size={32} />
          </div>
          <h3 class="text-lg font-bold text-gray-900 dark:text-white">
            {$_("users__delete_title")}
          </h3>
          <p class="text-sm text-gray-500 dark:text-gray-400 mt-2">
            {$_("users__delete_confirm_text", {
  ...
  {#if showAddModal}
    ...
    >
      <div
        class="absolute inset-0 bg-gray-900/60 backdrop-blur-sm animate-fade-in-quick"
      ></div>

      <div
        class="relative w-full max-w-lg bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden animate-zoom-in"
      >
        <!-- Modal Header -->
        <div
          ...
          <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
              {editingUser
                ...
                : $_("users__modal_subtitle")}
            </p>
          </div>
          <button
            on:click={resetForm}
            class="p-2 text-gray-400 hover:text-gray-600 dark:hover:text-white rounded-lg transition-colors"
          >
            <X size={24} />
          </button>
        </div>
          ...
          <form on:submit|preventDefault={handleInvite} class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-3 mb-2">
              <button
                ...
                class="flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl border transition-all {formData.role ===
                'user'
                  ? 'bg-indigo-500/10 border-indigo-500 text-indigo-500 font-bold'
                  : 'bg-gray-50 dark:bg-gray-900/50 border-gray-200 dark:border-gray-700 text-gray-500 hover:border-gray-300'}"
              >
                <UserIcon size={14} />
            ...
            {#if !editingUser}
              <div
                class="flex p-1 bg-gray-100 dark:bg-gray-950 rounded-xl mb-4"
              >
                <button
                  type="button"
                  on:click={() => (registrationMode = "invite")}
                  class="flex-1 py-2 text-xs font-bold rounded-lg transition-all {registrationMode ===
                  'invite'
            ...
            <div>
              <label
                for="email"
                class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1.5"
                >{$_("users__form_email")}</label
              >
              <div class="relative">
                <div
                ...
                <input
                  ...
                  class="w-full pl-11 pr-4 py-2.5 bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white placeholder-gray-400 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all {editingUser
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label
                  for="first_name"
                  class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1.5"
                  >{$_("users__form_first_name")}</label
                >
                <div class="relative">
                  <div
                    class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-gray-400"
            ...
            <div class="grid grid-cols-2 gap-4">
              ...
              <div>
                <label
                  for="position"
                  class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1.5"
                  >{$_("users__form_position")}</label
                >
                <div class="relative">
                  <div
                    class="absolute inset-y-0 left-0 pl-4 z-10 flex items-center pointer-events-none text-gray-400"
                  >
                    <Briefcase size={16} />
                  </div>
                  <SearchableSelect
            ...
            <div>
              <label
                for="discord_id"
                class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1.5"
                >{$_("users__form_discord_id")}</label
              >
              <div class="relative">
                <div
                  class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-gray-400"
                >
                  <div
                    class="w-4 h-4 rounded-full bg-indigo-500/20 flex items-center justify-center text-[8px] font-bold text-indigo-500"
                  >
                    D
                  </div>
                </div>
            ...
            {#if setupLink}
              <div
                class="bg-indigo-500/10 border border-indigo-500/20 p-4 rounded-xl animate-fade-in space-y-3"
              >
                <div
                  class="flex items-center gap-2 text-indigo-500 dark:text-indigo-400 text-sm font-bold border-b border-indigo-500/20 pb-2"
                >
                  <CheckCircle2 size={16} />
                  {$_("users__invite_link_generated")}
                </div>
                <p class="text-xs text-gray-500 dark:text-gray-400">
                  {$_("users__copy_link_hint")}
                </p>
                <div class="flex gap-2">
                  <input
                    readonly
                    value={setupLink}
                    class="flex-1 bg-white dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-lg py-1.5 px-3 text-xs text-indigo-600 dark:text-indigo-300 outline-none font-mono"
                  />
                  <button
                    type="button"
                    on:click={() => {
                    ...
                    class="bg-indigo-600 hover:bg-indigo-500 text-white px-3 py-1.5 rounded-lg text-xs font-medium transition-colors"
                    >{$_("users__btn_copy")}</button
                  >
                </div>
              </div>
            {/if}
            ...
            <div class="flex gap-3 pt-2">
              ...
              >
                ...
                {:else}
                  ...
                  {:else}
                    {registrationMode === "invite"
                      ? $_("users__btn_create_invite")
                      : $_("users__btn_create_user")}
                  {/if}
                  <ArrowRight
                    size={18}
  ...
  {#if showToast}
    <div class="fixed bottom-8 right-8 z-[2000] animate-zoom-in">
      <div
        class="bg-success text-white px-6 py-3 rounded-xl shadow-xl flex items-center gap-3 ring-1 ring-black/5 dark:ring-white/5"
      >
        <div
          class="flex items-center justify-center bg-white/20 rounded-full w-8 h-8 backdrop-blur-md"
        >

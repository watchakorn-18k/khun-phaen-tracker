use axum::{
    body::Body, extract::{Multipart, Path, State}, http::{header, StatusCode}, response::IntoResponse, Json
};
...
pub async fn upload_attachment(
    ...
    let ws_id = match ObjectId::parse_str(&ws_id_str) {
        Ok(id) => id,
        Err(_) => return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": "Invalid workspace ID"}))),
    };
    let task_id = match ObjectId::parse_str(&task_id_str) {
        Ok(id) => id,
        Err(_) => return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": "Invalid task ID"}))),
    };

    let client = match &state.storage_client {
        Some(c) => c,
        None => return (StatusCode::SERVICE_UNAVAILABLE, Json(serde_json::json!({"error": "Storage is disabled"}))),
    };
    ...
    if task.workspace_id != ws_id {
        return (StatusCode::FORBIDDEN, Json(serde_json::json!({"error": "Task does not belong to this workspace"})));
    }
    ...
    while let Some(field) = multipart.next_field().await.unwrap_or(None) {
        ...
        let upload_res = client.put_object()
    ...
    if attached_files.is_empty() {
        return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": "No valid files received"})));
    }
    ...
    if let Ok(attachments_bson) = mongodb::bson::to_bson(&current_attachments) {
        ...
        if let Err(e) = data_repo.update_task(&task_id, &ws_id, updates).await {
            ...
            return (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": "Failed to update db"})));
        }
    }

    (StatusCode::OK, Json(serde_json::json!({"success": true, "attachments": attached_files})))
}
...
pub async fn download_attachment(
    ...
    let resp = match client.get_object().bucket(&state.storage_bucket).key(&file_key).send().await {
        Ok(r) => r,
        Err(err) => {
...
pub async fn delete_attachment(
    ...
    let task = match data_repo.find_task_by_id(&task_id).await {
        Ok(Some(t)) => t,
        _ => return (StatusCode::NOT_FOUND, Json(serde_json::json!({"error": "Task not found"}))),
    };
    ...
    if let Some(file_key) = file_key_to_delete {
        if let Some(client) = &state.storage_client {
            let _ = client.delete_object().bucket(&state.storage_bucket).key(&file_key).send().await;
        }
        ...
        (StatusCode::NOT_FOUND, Json(serde_json::json!({"error": "Attachment not found"})))
    }
}

...
pub async fn archive_sprint_tasks(
    State(state): State<SharedState>,
    Path((ws_id, sprint_id)): Path<(String, String)>,
    headers: HeaderMap,
    jar: CookieJar,
    ...
    let repo = DataRepository::new(&state.db);
    match repo.archive_completed_sprint_tasks(&ws_oid, &spr_oid).await {
        Ok(count) => {
            axum::Json(serde_json::json!({ "success": true, "archived_count": count }))
                .into_response()
        }
        Err(e) => (
            axum::http::StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn move_incomplete_sprint_tasks(
    ...
    let repo = DataRepository::new(&state.db);
    match repo.move_incomplete_sprint_tasks(&ws_oid, &spr_oid).await {
        Ok(count) => {
            axum::Json(serde_json::json!({ "success": true, "moved_count": count }))
                .into_response()
        }
        Err(e) => (
            axum::http::StatusCode::INTERNAL_SERVER_ERROR,
